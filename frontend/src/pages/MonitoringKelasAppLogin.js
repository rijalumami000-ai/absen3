...
const MonitoringKelasAppLogin = () => {
  const [username, setUsername] = useState('');
  const [kodeAkses, setKodeAkses] = useState('');
  const [loading, setLoading] = useState(false);
  const { login } = usePembimbingKelasAuth();
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-700 via-green-600 to-green-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-green-100">Madrasah Diniyah</p>
        </div>
        ...
        <div className="text-center mt-6">
          <p className="text-green-100 text-sm">
            Aplikasi Monitoring Madrasah Diniyah
          </p>
        </div>
      </div>
    </div>
...
export default MonitoringKelasAppLogin;
