...
import { User, KeyRound, BarChart3 } from 'lucide-react';

const MonitoringAliyahAppLogin = () => {
  const [username, setUsername] = useState('');
  const [kodeAkses, setKodeAkses] = useState('');
  const [loading, setLoading] = useState(false);
  const { login, user, loading: authLoading } = useMonitoringAliyahAuth();
  const { toast } = useToast();
  const navigate = useNavigate();

  useEffect(() => {
    if (!authLoading && user) {
      navigate('/monitoring-aliyah-app');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      toast({
        ...
        description: 'Selamat datang di Monitoring Madrasah Aliyah',
      });
      navigate('/monitoring-aliyah-app');
    } else {
      toast({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-primary-100 flex flex-col" data-testid="monitoring-aliyah-login-page">
      <div className="absolute inset-0 opacity-5">
        <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          <pattern id="grid-aliyah-monitor" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse">
            <circle cx="5" cy="5" r="1" fill="currentColor" className="text-primary-700" />
          </pattern>
          <rect width="100%" height="100%" fill="url(#grid-aliyah-monitor)" />
        </svg>
      </div>
      ...
      <div className="relative flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-3xl font-bold text-foreground">Monitoring Aliyah</h1>
            <p className="text-muted-foreground mt-2">Aplikasi Pemantau Absensi Madrasah Aliyah</p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Username Monitoring Aliyah"
                    required
          ...
          <p className="text-center text-xs text-muted-foreground mt-6">
            Sistem Monitoring Absensi Madrasah Aliyah
          </p>
        </div>
      </div>
    </div>
...
export default MonitoringAliyahAppLogin;
